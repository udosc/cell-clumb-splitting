...
public class CopyOfCurvatureSplitting<T extends RealType<T> & NativeType<T>, L extends Comparable<L>> {
	...
	public CopyOfCurvatureSplitting(ContourDistance<T> distance, Img<BitType> img, Contour... templates){
		m_templates = Arrays.asList( templates );
		m_img = img;
		m_distance = distance;
	}
	
	public CopyOfCurvatureSplitting(ContourDistance<T> distance, Img<BitType> img,List<Contour> templates){
...
//				}
				...
				res *=  ( m_contour[i][j].size() ) / Math.abs( (double)curvature.dimension(0) ) ;
				...
				for(int k = i+1; k < j; k++){
					for(int l = k + 1; l < j; l++){
						...
						if( dist < m_weights[i][k] + m_weights[l][j] ){
							m_weights[i][k] = tmp * ( m_contour[i][k].size() ) / Math.abs( (double)curvature.dimension(0) ) ;
							m_weights[l][j] = tmp * ( m_contour[l][j].size() ) / Math.abs( (double)curvature.dimension(0) );
//							m_split[][]
						}
					}
				}
