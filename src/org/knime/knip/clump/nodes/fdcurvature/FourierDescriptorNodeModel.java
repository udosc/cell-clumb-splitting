 ...
 * @author Schlegel
 */
public class FourierDescriptorNodeModel<T extends RealType<T> & NativeType<T>, L extends Comparable<L>> 
	extends NodeModel {
    
	private static final int NUMBER_OF_POINTS = 128;
    ...
    private List<String[]> m_ftCurvatures;
     ...
     */
    protected FourierDescriptorNodeModel() {
        super(1, 1);
    }
...
//    	final int numberOfImgs = inData[0].getRowCount();
    	...
    	m_ftCurvatures = new LinkedList<String[]>(  );
    	...
    	for (DataRow row: inData[0]) {
    		...
    		for(Pair<L, long[]> start: map){
    			...
    			final FourierOfCurvature<DoubleType> fd = new FourierOfCurvature<DoubleType>( curvature , NUMBER_OF_POINTS);
    			String[] res = new String[ m_numberOfFD.getIntValue() ];
    			Complex[] c = fd.getDescriptors(m_numberOfFD.getIntValue(), false);
    			double[] d = fd.getMagnitudes(m_numberOfFD.getIntValue(), false);
    			for(int i = 0; i < res.length; i++){
    				res[i] = d[i] + " - "  + c[i].toString();
    			}
    			m_ftCurvatures.add(res);
    		}
        }
    	
    	DataColumnSpec[] allColSpecs = new DataColumnSpec[  m_numberOfFD.getIntValue() ];
        for(int i = 0; i < allColSpecs.length; i++)
        	allColSpecs[i] = 
        		new DataColumnSpecCreator( i+"", StringCell.TYPE).createSpec();
    	
    	
        BufferedDataContainer container1 = exec.createDataContainer( 
    			new DataTableSpec( allColSpecs ));
        int i = 0;
    	for(String[] s: m_ftCurvatures ){
    		DataCell[] cells = new DataCell[ m_numberOfFD.getIntValue() ];
    		
    		for(int j = 0; j <  s.length; j++){
    			cells[j] = new StringCell( s[j] );
    		}
